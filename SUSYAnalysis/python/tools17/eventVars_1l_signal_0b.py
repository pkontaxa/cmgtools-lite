...
C_ISRweightsSusy = {}
cntFileT1 = "../python/tools17/t1ttt_scan_counts.txt"
cntFileT5 = "../python/tools17/t5qqqq_scan_counts.txt"
...
ISRweightFile = "../python/tools17/ISRnormWeightsT5qqqqWW_17_final.txt"
...
class EventVars1L_signal_0b:
    ...
    def __call__(self,event,base):
        ...
        # output dict:
        ret = {}
        if (not event.isData and "T5qqqq" in self.sample) :
            ...
            if "T5qqqq" in self.sample :
                # lis1 = []
                # with open(ISRweightFile, "r") as file_short:
                    # for line in file_short.readlines():
                        # if not "m" in line.split("\t")[0]:
                            # lis1.append(line.split("\t")[:2])
                ...
                #if [mGo, mLSP] not in lis1:
                #    print("not found in C_ISRweightsSusy:", ((mGo,mLSP)))
                #    print( "keys" , C_ISRweightsSusy.keys())
                #    C_ISRweightsSusy[(int(mGo),int(mLSP))] = (1,0,0)
                if (mGo, mLSP) not in C_ISRweightsSusy.keys():
                    print("not found in C_ISRweightsSusy:", ((mGo,mLSP)))
                    C_ISRweightsSusy[(mGo,mLSP)] = ('1','1','1')
                ...
                print("debug", mGo, mLSP, C_ISR, C_ISR_up, C_ISR_down)
        ...
        #else : print('sample is not signal sample')
        return ret
