...
# for signal masses
from CMGTools.SUSYAnalysis.tools.eventVars_1l_signal_Bkg_Flat_mGo_mLSP_V2 import EventVars1L_signal
...
from CMGTools.TTHAnalysis.tools.BDT_Limits_HT500_LT250_nj5_dphi075_nb2_Parametrized_Top_Properties_mass_difference import BDT_Limits_Parametrized
MODULES.append( ('BDT_Limits_Parametrized',  BDT_Limits_Parametrized(os.environ["CMSSW_BASE"]+"/src/CMGTools/TTHAnalysis/data/kinMVA/tth/TMVAClassification_BDT_HT500_LT250_nj5_dphi075_nb2_Parametrized_Top_Properties_mass_difference_8M.weights.xml")) )
...
if options.naf:
    ...
    for (name,fin,fout,data,range,chunk) in jobs:
        if chunk != -1:
            ...
            #Pantelis jobList.write("{base} -d {data} {post}".format(base=basecmd, data=name, chunk=chunk, post=friendPost)+'\n')
        else:
            #print "{base} -d {data} {post}".format(base=basecmd, data=name, chunk=chunk, post=friendPost)
    ...
    if options.bulk : 
        ...
        #Pantelis temp = temp.replace('@EXESH',str(os.getcwd())+'/$(Chunk)/wrapnanoPost.sh').replace('@LOGS',str(logdir)).replace('@time','60*60*3').replace('Queue 1','queue Chunk matching dirs '+outdir+'/*')
        temp = temp.replace('@EXESH','/$(Chunk)/wrapnanoPost.sh').replace('@LOGS',str(logdir)).replace('@time','60*60*3').replace('Queue 1','queue Chunk matching dirs '+outdir+'/*')
