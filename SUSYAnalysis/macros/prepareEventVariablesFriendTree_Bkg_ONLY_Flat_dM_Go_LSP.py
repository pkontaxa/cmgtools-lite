...
if options.naf:
    ...
    for line in listtxt: 
        ...
        if not options.bulk : 
            ...
            #Pantelis subCmd = 'condor_submit -name s02 '+condsub
    ...
    if options.bulk : 
        ...
        temp = temp.replace('@EXESH','/$(Chunk)/wrapnanoPost.sh').replace('@LOGS',str(logdir)).replace('@time','60*60*6').replace('Queue 1','queue Chunk matching dirs '+outdir+'/*')
        temp_toRun =  open('condor.sub_all', 'w')
        temp_toRun.write(temp)
        temp_toRun.close()
    if  os.path.exists('condor.sub_all'):
        #Pantelis os.system('condor_submit -name s02 condor.sub_all')
