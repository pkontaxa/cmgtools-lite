...
void @SCRIPTNAME( TString myMethodList = ""  )
{
   ...
   Float_t puRatio,lepSF,Xsec,genWeight,susyXsec,susyNgen,nISRweight,nISRttweight,nJets30Clean,mGo,mLSP,nTrueInt,b_BDTC;
   ...
   for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
      if (it->second) {
         ...
         TString weightfile = "/nfs/dust/cms/user/amohamed/susy-desy/nanoAOD/pureNANOAOD/CMSSW_9_4_4/src/tthAnalysis/NanoAODTools/TMVA/datasetBkg_v6_uncomp/weights/TMVAClassification_BDT.weights.xml";
   ...
   theTree->SetBranchAddress("b_BDTC",&b_BDTC);
   ...
   t->Branch("b_BDT",  &b_BDT); 
   t->Branch("b_BDTC",&b_BDTC);
