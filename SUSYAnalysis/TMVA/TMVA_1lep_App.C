...
void TMVA_1lep_App( TString myMethodList = ""  )
{

   //---------------------------------------------------------------
   // This loads the library
   TMVA::Tools::Instance();
   TString inFile = "/nfs/dust/cms/user/amohamed/susy-desy/CMGSamples/Friends_for_Limits/evVarFriend_WZTo1L3Nu.root";
   ...
   for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
      if (it->second) {
         ...
         TString weightfile = "/nfs/dust/cms/user/amohamed/susy-desy/nanoAOD/pureNANOAOD/CMSSW_9_4_4/src/tthAnalysis/NanoAODTools/TMVA/datasetBkg_0/weights/TMVAClassification_BDT.weights.xml";
   ...
   TFile *target  = new TFile( "evVarFriend_WZTo1L3Nu.root","RECREATE" );
   Float_t b_BDT;
   TDirectory *sfdir;
