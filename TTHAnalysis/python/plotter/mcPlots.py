...
def doRatioHists(pspec,pmap,total,totalSyst,maxRange,fixRange=False,fitRatio=None,errorsOnRef=True,ratioNums="signal",ratioDen="background",ylabel="Data/pred.",doWide=False,showStatTotLegend=False):
    ...
    unity.GetYaxis().SetRangeUser(0,rmax);
...
legend_ = None;
def doLegend(pmap,mca,corner="TL",textSize=0.035,cutoff=0.0,cutoffSignals=True,mcStyle="F",legWidth=0.18,legBorder=True,signalPlotScale=None,totalError=None,header="",doWide=False,sf=None,columns=1):
		...
		if corner == "TR":
			(x1,y1,x2,y2) = (0.97-legWidth if doWide else .72-legWidth, .9 - height, .90, .91)
		elif corner == "TC":
		...
		#Pantelis if options.showSF: leg.AddEntry('SF', 'SF: {0}'.format(round(1.19,2)), '')
		leg.Draw()
    ...
    def run(self,mca,cuts,plots,makeStack=True,makeCanvas=True):
        ...
        for subname, title, cut in sets:
            ...
            for pspec in pspecs:
                ...
                xblind = [9e99,-9e99]
                print self._options.unblind    
                #xblind = [0.85,1.]   
                print blind    
                print "TEST1"
                print re.match(r'(bin|x)\s*([<>]?)\s*(\+|-)?\d+(\.\d+)?|(\+|-)?\d+(\.\d+)?\s*<\s*(bin|x)\s*<\s*(\+|-)?\d+(\.\d+)?', blind)
                if re.match(r'bin', blind):
                    print "TEST2"
                    ...
                    hdata = pmap['data']                            
                    #print xrange(1,hdata.GetNbinsX()+1)
                    for b in xrange(1,hdata.GetNbinsX()+1):
                        print "TEST3" 
                        print b 
                        print hdata.GetXaxis().GetBinCenter(b)
                        print test(xfunc(hdata,b)) 
                        #if test(xfunc(hdata,b)):
                        if(0.8 <= hdata.GetXaxis().GetBinCenter(b) <= 1.): 
               ...
               # norm to bin width
                if self._options.normBinW:
                    for p in mca.listProcesses():
                        if p in pmap:
                            plot = pmap[p]
                            if 'TH1' in plot.ClassName(): plot.Scale2(0.1,"width") #Pantelis: new Scale2() function
...
#                        if plot.Integral() <= 0: continue
                ...
                legendCutoff = pspec.getOption('LegendCutoff', 0.0 if c1.GetLogy() else 0.0)
                if plotmode == "norm": legendCutoff = 0 
                if plotmode == "stack":
                    if options.noStackSig: mcStyle = ("L","F")
                    else:                  mcStyle = "F"
                else: mcStyle = "L"
                doLegend(pmap,mca,corner=pspec.getOption('Legend','TR'),#Pantelis
                                  ...
                                  doWide=doWide, totalError=totalError, sf=self._sf,columns = pspec.getOption('LegendColumns',options.legendColumns))
                                  #Pantelis doWide=doWide, totalError=totalError, columns = pspec.getOption('LegendColumns',options.legendColumns))  
                        ...
                        if ext == "txt":
                            ...
                            if "TH2" in total.ClassName() or "TProfile2D" in total.ClassName():
                                ...
                                for p in mca.listSignals(allProcs=True) + mca.listBackgrounds(allProcs=True) + ["signal", "background", "data", "total"]:
                                    ...
                                    #Pantelis plot.SetMarkerStyle(mca.getProcessOption(p,'MarkerStyle',1))
                                    #Pantelis plot.SetMarkerColor(mca.getProcessOption(p,'FillColor',ROOT.kBlack))
