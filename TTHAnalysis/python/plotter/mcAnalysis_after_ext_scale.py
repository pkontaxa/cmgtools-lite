...
class MCAnalysis:
    ...
    def readMca(self,samples,options):
        ########## Pantelis #####################
        cnames_tot =[]  
        for line_tot in open(samples,'r'):
            if re.match("\s*#.*", line_tot): continue
            line_tot = re.sub(r"(?<!\\)#.*","",line_tot)  ## regexp black magic: match a # only if not preceded by a \! 
            line_tot = line_tot.replace(r"\#","#")        ## and now we just unescape the remaining #'s

            extra_tot = {}
            if ";" in line_tot:
                (line_tot,more_tot) = line_tot.split(";")[:2]
                for setting in [f.replace(';',',').strip() for f in more_tot.replace('\\,',';').split(',')]:
                    if setting == "": continue
                    if "=" in setting:
                        (key,val) = [f.strip() for f in setting.split("=",1)]
                        extra_tot[key] = eval(val)
                    else: extra_tot[setting] = True
            field_tot = [f.strip() for f in line_tot.split(':')]
            if len(field_tot) == 1 and field_tot[0] == "*":
                ...
                #for k,v in extra_tot.iteritems():
                    #print "\t%s: %r" % (k,v)
                    #self.init_defaults_tot[k] = v
                #continue
            #else:
                #for k,v in self.init_defaults_tot.iteritems():
                    #if k not in extra_tot: extra_tot[k] = v
            if len(field_tot) <= 1: continue
            if "SkipMe" in extra_tot and extra_tot["SkipMe"] == True and not options.allProcesses: continue
            signal = False
            pname_tot = field_tot[0]
            if pname_tot[-1] == "+":
                #signal = True
                pname_tot = pname_tot[:-1]
            cname_tot = [ x.strip() for x in field_tot[1].split("+") ]
            cnames_tot.append((pname_tot, cname_tot))
        #for i, x in enumerate (cnames_tot):
        #      if "data" not in cnames_tot[i][0]:  
           ...
           ## If we have a user-defined list of processes as signal
            ...
            for cname in cnames:
                ...
                if "data" not in pname:
                    ...
                    ############ Pantelis for ext samples! ###########################
		    smaller_list = []
                    if "_ext" in cname:
                        nom = cname.replace(cname[cname.find("_ext"):],'')
                    else:
                        nom = cname
                    #print nom 
                    for i, x in enumerate(cnames_tot):
				...
				smaller_list.append(cnm_tot[0])                	
                    ...
                    for i, x in enumerate(smaller_list):
                         if (cname!=smaller_list[i]):
                               pckfile_tot = basepath+"/%s/skimAnalyzerCount/SkimReport.pck" % smaller_list[i]
                               pckobj_tot  = pickle.load(open(pckfile_tot,'r'))
                               counters_total = dict(pckobj_tot)
			       if ('Sum Weights' in counters_total) and options.weight:
                                        total_w += counters_total['Sum Weights']
                               else:
                                        total_w += counters_total['All Events']   	
                    ################################################################## 
