...
def decryptBinName(binname):
    ...
    elif "LT4i" in binname: binname = binname.replace("LT4i","L_{T} #geq 650")

    '''
    #elif "LT1" in binname: binname = binname.replace("LT1","L_{T} #in [250,350]")
    elif "LT12" in binname: binname = binname.replace("LT12","L_{T} #in [250,450]")
    ...
    elif "LT4i" in binname: binname = binname.replace("LT4i","L_{T} #geq 600")
    elif "LT4" in binname: binname = binname.replace("LT4","L_{T} #in [600,750]")
    elif "LT5i" in binname: binname = binname.replace("LT5i","L_{T} #geq 750")
    '''
...
def getQCDfromFit(dhData, hQCD, hEWK, var):
    ...
    print "########################################################################################## pdfQCD.GetName(): ",pdfQCD.GetName()
    ...
    print "################### nQCD: ", nQCD.getValV()
    print "################### nEWK: ", nEWK.getValV()
...
def getHistsFromFile(tfile, binname = 'incl', mcData = True):
    ...
    if mcData:
        ...
        #QCDantiName = 'Lp_sel_'+binname+'_QCD'
    ...
    ##################################
    if not mcData:
    	EWKantiName = 'Lp_anti_'+binname+'_background'
        QCDantiNameMC = 'Lp_anti_'+binname+'_QCD'
    	hEWKanti = tfile.Get(EWKantiName).Clone('EWKanti_'+binname) 	    
        hQCDantiMC = tfile.Get(QCDantiNameMC).Clone('QCDantiMC_'+binname)
...
def plotHists(binname = 'incl', inclTemplate = False, mcData = True, addHists = True):
    ...
    if not inclTemplate:
        argset = RooArgSet(_pdfStore['pdfQCDsel_'+binname]) # hack to keep arguments alive
    else:
        import re
        if (binname[3]!='_'):
            if 'i' in binname:
                 incName = re.sub('LT[0-9]i','LTi',binname)
            else:
                incName = re.sub('LT[0-9][0-9]','LTi',binname)
        else:                     
        ...
        if 'pdfQCDsel_'+incName not in _pdfStore: incName = re.sub('LT[0-9]_','',binname) # remove LTx to get incl template
        #argset = RooArgSet(_pdfStore['pdfQCDanti_'+incName]) # hack to keep arguments alive
        #argset = RooArgSet(_pdfStore['pdfQCDanti_'+incName]) # hack to keep arguments alive
        argset = RooArgSet(_pdfStore['pdfQCDsel_'+incName]) # hack to keep arguments alive
    ...
    if addHists:
        ...
        _hStore['EWKsel_'+binname].Draw("histsame")
    ...
    print "###########: ", (frame.findObject('QCDfit').Integral())
    leg.AddEntry(frame.findObject('FullFit'),'Full fit','l')
    leg.AddEntry(frame.findObject('QCDfit'),'QCD fit (MC)','l')
...
def getQCDratio(tfile, options, binname = 'incl'):
    ...
    if options.inclTemplate:
        ...
        if 'QCDsel_'+incName not in _hStore: incName = re.sub('LT[0-9]_','',binname) # remove LTx to get incl template
        if 'QCDsel_'+incName not in _hStore: print "Didn't find inclusive template!";

        if options.verbose > 0:
            print 'Using template', incName, 'instead of', binname
        hQCDsele = _hStore['QCDsel_'+incName]


    # Get QCD prediction in the selected region
    nQCD = getQCDfromFit(data,hQCDsele,hEWKsele,lp)
...
if __name__ == "__main__":
    ...
    binNames = ['LTi_NJ34', 'LT1_NJ34', 'LT2_NJ34', 'LT3_NJ34', 'LT4i_NJ34'] #for 0-b
    ...
    # Moriond '17
    ##binNames = ['LTi_NJ34','LT12_NJ34','LT3i_NJ34','LT3_NJ34','LT4i_NJ34','LT4_NJ34','LT5i_NJ34']
