...
def combineCards(f, s ):
    ...
    cmd = 'combineCards.py ' + f + '/LT*txt > ' + f.replace(s,'combinedCardsAllYears') +'/' + s + '.txt'
    print cmd
    os.system(cmd)
...
def createJobs(f, s, jobs, useAsimov = True):
    cmd = 'combine -M AsymptoticLimits ../' + f + ' -n ' + s + '\n'
    if useAsimov:
        cmd = 'combine -t -1 -M AsymptoticLimits ../' + f + ' -n ' + s + '\n'
    #cmd = 'combine ../' + f + ' -n ' + s + '\n'
    print cmd
    jobs.write(cmd)
    return 1
...
if __name__ == "__main__":
    ...
    if "--combine-cards" in sys.argv:
        ...
        #print fileList
        try:
            os.stat('limitOutputRun2')
        except:
            os.mkdir('limitOutputRun2')

        chunks = 0
        os.chdir('limitOutputRun2')
