...
if __name__ == "__main__":
    ...
    yds34 = YieldStore("lepYields")
    yds45_nb0 = YieldStore("lepYields")
    yds45_nb1i = YieldStore("lepYields")
    ...
    for folder in ['grid-dilep', 'scan']:
        ...
        globfiles = b+"*NJ8i*"
        yds8.addFromFiles(globfiles,("lep","sele"))
        globfiles = b+"*NJ5*"
        yds5.addFromFiles(globfiles,("lep","sele"))

        b = a + 'mergedNJ34/LT'
        globfiles = b+"*NJ34*"
        yds34.addFromFiles(globfiles,("lep","sele"))

        b = a + 'mergedNJ45NB0/LT'
        globfiles = b+"*NB0_NJ45*"
        yds45_nb0.addFromFiles(globfiles,("lep","sele"))

        b = a + 'mergedNJ45NB1i/LT'
        globfiles = b+"*NB1i_NJ45*"
        yds45_nb1i.addFromFiles(globfiles,("lep","sele"))

        #b = a + 'mergedFew/LT'
        #globfiles = b+"*NJ67*"
        #ydsFew6.addFromFiles(globfiles,("lep","sele"))
        #globfiles = b+"*NJ8*"
        #ydsFew8.addFromFiles(globfiles,("lep","sele"))

    yds8.showStats()

    # collect all yds objects
    yds_list = [yds6, yds5, yds8, yds34, yds45_nb0, yds45_nb1i]
    ...
    # FIXME , works  I guess? but don't know why it was written like that
    if 1 ==1:
...
#                     ('T1tttt_Scan_mGo1500_mLSP100',cat),('T1tttt_Scan_mGo1200_mLSP800',cat)]
            ...
            yds8.printLatexTable(samps, printSamps, label, f, doSys)
...
#                  'background CR\_SB','T1t$^4$ 1.8$/$0.1 CR\_SB','T1t$^4$ 1.2$/$0.8 CR\_SB',]
    ...
    yds8.divideTwoYieldDictsForRatio('DLCR_MB','SR_MB','DLCR_MB_RelSize')
    yds8.divideTwoYieldDictsForRatio('DLCR_SB','SR_SB','DLCR_SB_RelSize')

    yds6.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'background','data')
    yds8.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'background','data')

    yds6.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'DiLep_Inc','background')
    yds8.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'DiLep_Inc','background')

    yds6.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'TTdiLep','background')
    yds8.divideTwoYieldDictsForRatio('COPYALL','COPYALL','dummy', False, 'TTdiLep','background')
...
#    OutputHelperList.append(OutputHelper(('data','DLCR_MB_RelSize'),"rel. size (DLCR)data", "percentage"))
    ...
    yds8.printLatexTableEnh(OutputHelperList, label, f)
    printLatexFooter(f)
    '''
    ...
    ydsFew8.printLatexTable(samps, printSamps, label, f, doSys, True)
    printLatexFooter(f, 1)
    f.close()
    ...
    printSamps = ['MC 8ij','T1tttt 1.9/0.1','T1tttt 1.4/1.1']
    ydsFew8.printLatexTable(samps, printSamps, label, f, False)
    ...
    printSamps = ['$R_{CS}$ [6,7] jets','$R_{CS}$ [4,5] jets','$\kappa$']#, '(Data-QCD) $R_{CS}$ [4,5] jets']
    samps = [('EWK','Rcs_MB'),('EWK','Rcs_SB'),('EWK','Kappa')]#,('data_QCDsubtr','Rcs_SB')]
    printLatexHeader(len(samps), f, caption,0)
    yds6.printLatexTable(samps, printSamps, label,f, doSys)
    printSamps = ['$R_{CS}$ $\geq$ 8 jets','$R_{CS}$ [4,5] jets','$\kappa$']#,'(Data-QCD) $R_{CS}$ [4,5] jets']
    samps = [('EWK','Rcs_MB'),('EWK','Rcs_SB'),('EWK','Kappa')]#,('data_QCDsubtr','Rcs_SB')]
    yds8.printLatexTable(samps, printSamps, label,f, doSys)
    printLatexFooter(f,0)
    f.close()
