...
#def printABCDCard(yds, ydsMuon, ydsObs, ydsMuObs, ydsPred, ydsKappaTT, ydsKappaW, ydsSigSys, year, yieldFractionDf, sysTableBttDf , sysTableWDf, sysTableSignalDf ):
def printABCDCard(yds, ydsMuon, ydsObs, ydsMuObs, ydsPred, ydsKappaTT, ydsKappaW, year, yieldFractionDf, sysTableBttDf , sysTableWDf, sysTableSignalDf, sysTableOtherDf):
    ...
    for i,bin in enumerate(bins):
        ...
        systSpaceBuffer = len("DLConst_2016_run2")
        doSystematics = True
        if doSystematics:
            for syst in sysTableBttDf.columns:
                ...
                for cat, samp in zip(catNames, sampNames):
                    if samp == "TTJets":
                        if cat == "SR_MB":
                            #systLine += " " + kpatt % sysTableBttDf.loc[bin, syst]
                            systVal = "1.000"
                        ...
                        if cat == "SR_MB":
                            ...
                            if float(systVal) == 1.0:
                                ...
                                systLine += " " + kpatt % systVal
                            #systLine += " " + kpatt % systVal
                            #systLine += " " + kpatt % sysTableWDf.loc[bin, syst]
                        else:
                            systLine += " " + kpatt % "-"
                    elif SMS in samp:
                        systVal = "1.000"
                        if bin in sysTableSignalDf.index and syst + "_" + cat in sysTableSignalDf.columns:
                            systVal = sysTableSignalDf.loc[bin, syst + "_" + cat]
                        if float(systVal) == 1.0:
                            systLine += " " + kpatt % "-"
                        else:
                            systLine += " " + kpatt % systVal
...
if __name__ == "__main__":
    ...
    yds5.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ5*", ("lep","sele"))
    yds67.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ67*", ("lep","sele"))
    yds8.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ8i*", ("lep","sele"))
    ydsMu5.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ5*", ("mu","sele"))
    ydsMu67.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ67*", ("mu","sele"))
    ydsMu8.addFromFiles("yield_0b_nosyst_" + year + "/*/merged/LT*NJ8i*", ("mu","sele"))

    #yds5.addFromFiles("yield_0b_" + year + "/*/merged/LT1_HT0_NB0_NJ5_NW0*", ("lep","sele"))
    #yds67.addFromFiles("yield_0b_" + year + "/*/merged/LT4i_HT3i_NB0_NJ67_NW0*", ("lep","sele"))
    #yds8.addFromFiles("yield_0b_" + year + "/*/merged/LT1_HT02_NB0_NJ8i_NW0*", ("lep","sele"))
    #ydsMu5.addFromFiles("yield_0b_" + year + "/*/merged/LT1_HT0_NB0_NJ5_NW0*", ("mu","sele"))
    #ydsMu67.addFromFiles("yield_0b_" + year + "/*/merged/LT4i_HT3i_NB0_NJ67_NW0*", ("mu","sele"))
    #ydsMu8.addFromFiles("yield_0b_" + year + "/*/merged/LT1_HT02_NB0_NJ8i_NW0*", ("mu","sele"))
    ...
    for mGo in [1500, 1900]:
        for mLSP in [0, 100]:
            #for ydIn, ydMuIn in [(yds5, ydsMu5), (yds67, ydsMu67)]:
            for ydIn, ydMuIn in ((yds5, ydsMu5), (yds67, ydsMu67) , (yds8, ydsMu8)):
                ...
                #cats = ('SR_MB', 'CR_MB', 'SR_SB','CR_SB', 'SR_SB_NB0', 'CR_SB_NB0', 'SR_SB_NB1i', 'CR_SB_NB1i')
                #catsNoSR = ('CR_MB', 'SR_SB','CR_SB', 'SR_SB_NB0', 'CR_SB_NB0', 'SR_SB_NB1i', 'CR_SB_NB1i')
                cats = ('SR_MB', 'CR_MB', 'SR_SB','CR_SB', 'SR_SB_NB1i', 'CR_SB_NB1i')
                catsNoSR = ('CR_MB', 'SR_SB','CR_SB', 'SR_SB_NB1i', 'CR_SB_NB1i')

                #sampsABCDbkg = [("TTJets", cat) ("WJets", cat) for cat in catsNoSR]
                sampsABCDbkg = []
                sampsABCDObs = []
                for cat in cats:
                ...
                #sampsABCDSigSys = [
                #    (SMS+'_Scan_JEC_syst_mGo'+str(mGo)+'_mLSP'+str(mLSP), 'SR_MB'),
                ...
                #ydsPredABCD = ydIn.getMixDict([("TTJets_pred", "SR_MB")])
                #ydsMuPredABCD = ydMuIn.getMixDict([("WJets_pred", "SR_MB")])
                ydsPredABCD = ydIn.getMixDict([("TTJets_pred", "SR_MB"), ("WJets_pred", "SR_MB")])


                #ydsKappaTT = ydIn.getMixDict([("TTJets", "KappaB"), ("TTJets", "KappaTT"), (prefix + "_QCDpred", "CR_MB"), (prefix + "_QCDpred", "CR_SB_NB1i"), (prefix + "_QCDpred", "SR_SB_NB1i")])
                ...
                printABCDCard(ydsABCD, ydsMuonABCD, ydsObsABCD, ydsMuonObsABCD, ydsPredABCD, ydsKappaTT, ydsKappaW, year = year, yieldFractionDf = yieldFractionDf, sysTableBttDf = sysTableBttDf, sysTableWDf = sysTableWDf, sysTableSignalDf = sysTableSignalDf, sysTableOtherDf = sysTableOtherDf)
