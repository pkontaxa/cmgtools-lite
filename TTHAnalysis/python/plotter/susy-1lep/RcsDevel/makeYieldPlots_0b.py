...
colorDict = {'TTJets': kBlue-4,'TTdiLep':kBlue-4,'TTsemiLep':kBlue-2,'WJets':kGreen-2,
             'TT + jets (Pred)': kBlue-4,'W + jets (Pred)':kGreen-2,
             'TT+jets (Pred)': kBlue-4,'W+jets (Pred)':kGreen-2,
             'TTJets (Pred)': kBlue-4,'WJets (Pred)':kGreen-2,
             'RCS_MB_TT' : kBlue+2,'kappaTT' : kBlue+3,'kappaB' : kOrange+2,
             'RCS_SB_NB1i_TT' : kBlue+2,
             'RCS_MB_TT_MC' : kGreen,
             'RCS_SB_NB1i_TT_MC' : kRed,
             'RCS_SB_NB0_TT_MC' : kOrange,
             'RCS_MB_Wneg' : kGreen+2,'kappaW_neg' : kGreen+3,
             'RCS_SB_Wneg' : kGreen+2,
             'RCS_MB_Wneg_MC' : kBlue,
             'RCS_SB_Wneg_MC' : kRed,
             'RCS_MB_Wpos' : kGreen+2,'kappaW_pos' : kGreen+3,
             'RCS_SB_Wpos' : kGreen+2,
             'RCS_MB_Wpos_MC' : kBlue,
             'RCS_SB_Wpos_MC' : kRed,
             'RCS_MB' : kBlue+2,'kappaTT' : kBlue+3,'kappaB' : kOrange+2,
             'QCD':kCyan-6,'SingleT':kViolet+5,'DY':kRed-6,'TTV':kOrange-3,'data':1,'background':2,'EWK':3,'VV':kRed+3}
...
def doLegend(pos = "TM",nEntr = None):
    ...
    if pos == "TM":
        ...
        leg = TLegend(0.2,0.82,0.95,0.92) # Top
        #leg = TLegend(0.2,0.85,0.85,0.45) # Bottom
    elif pos == "Wide":
...
def getSampLabel(name):
    ...
    names = {
        "TT" : "t#bar{t} + jets",
        "TTJets" : "t#bar{t} + jets",
        "TTJets (Pred)" : "t#bar{t} + jets (Pred)",
        "TTsemiLep" : "t#bar{t} (1l) + jets",
        "TTdiLep" : "t#bar{t} (2l) + jets",
        "SingleTop" : "t/#bar{t}",
        "SingleT" : "t/#bar{t}",
        "WJets" : "W + jets",
        "WJets (Pred)" : "W + jets (Pred)",
        "DY" : "DY + jets",
        "QCD": "QCD",
        "TTV": "t#bar{t}V",
        "T5qqqqWW_Scan_mGo1500_mLSP1000": "T5qqqqWW (1.5, 1.0)",
        "T5qqqqWW_Scan_mGo1900_mLSP100": "T5qqqqWW (1.9, 0.1)",
        'T5qqqqWW_Scan_mGo2150_mLSP1750' : 'T5qqqqWW (2.15, 1.75)',
        'T5qqqqWW_Scan_mGo2100_mLSP1800' : 'T5qqqqWW (2.10, 1.8)',
        'T5qqqqWW_Scan_mGo2150_mLSP1800' : 'T5qqqqWW (2.15, 1.8)',
        'T5qqqqWW_Scan_mGo1050_mLSP900' : 'T5qqqqWW (1.05, 0.9)',
        'T5qqqqWW_Scan_mGo1800_mLSP200' : 'T5qqqqWW (1.80, 0.2)',
        'T5qqqqWW_Scan_mGo2400_mLSP1400' : 'T5qqqqWW (2.4, 1.4)',
        'T5qqqqWW_Scan_mGo2550_mLSP200' : 'T5qqqqWW (2.55, 0.2)',
        }
...
def makeSampHisto(yds, samp, cat, hname = "", ind = 0, useRcs = True): # False
    ...
    if "MB" in cat or useRcs:
        ...
        for njbin in ['NJ34', 'NJ45']:
            bBinList = []
            if njbin == 'NJ34':
                bBinList = ['NB0']
            if njbin == 'NJ45':
                bBinList = ['NB0', 'NB1i']
            for bbin in bBinList:
                binList += [b for b in sorted(ydict.keys()) if njbin in b and bbin in b]
    else:
        print("Invalid category {} chosen! Exiting now..".format(cat))
        exit()
    ...
    if "Rcs" in cat:
        samp_tmp = samp
        if "NB1i" in cat and samp_tmp == "TTJets":
            samp_tmp = "EWK"
        if "SB_W" in cat and samp_tmp == "WJets":
            samp_tmp = "EWK"
        #htitle = cat.replace("Rcs_","R_{CS}^{") + "} (%s)" %samp
        htitle = cat.replace("Rcs_","R_{CS}^{") + "} (%s)" %samp_tmp
    else:
        #htitle = cat + " (%s)" %samp
        htitle = "%s" %samp
    ...
    for ibin,bin in enumerate(binList):
        ...
        #Pantelis binLabel = getCleanLabel(binLabel)

        #if binLabel in ulabs: binLabel = ulabs[binLabel]

        newLabel = "#splitline"
        binLabel = binLabel.replace("_Few","")
        binLabel = binLabel.replace("_forTTJets","")
        binLabel = binLabel.replace("_forWJets","")
        binLabel = binLabel.replace("_forWJets","")
        binLabel = binLabel.replace("_pos","")
        binLabel = binLabel.replace("_neg","")
...
def prepRatio(hist, keepStyle = False):
    ...
    hist.GetXaxis().SetLabelSize(0.08)#Pantelis
...
def getRatio(histA,histB, keepStyle = False):
    ...
    hRatio.GetXaxis().SetLabelSize(0.08) #Pantelis
...
def getCatLabel(name):
    ...
    # Signal
    cname = cname.replace("T5qqqqWW Scan","T5qqqqWW")
    if "mLSP" in cname:
...
def plotHists(cname, histList, ratio = None, legPos = "TM", width = 800, height = 600, logY = False, nCols = 3, mergeFolder = "merged"):
    ...
    if ratio != None:
        ...
        # plot bins separator
        #binlabel_ratio=hRatio.GetXaxis().GetBinLabel(1)
        #print binlabel_ratio
        ...
        if "kappa" in rname:
            hRatio.GetYaxis().SetTitle("#kappa")
            hRatio.GetYaxis().CenterTitle()
            hRatio.GetYaxis().SetNdivisions(505)
            hRatio.GetYaxis().SetTitleSize(0.10)
            hRatio.GetYaxis().SetTitleOffset(0.3)
            ...
            hRatio.GetXaxis().SetLabelOffset(0.0025)
...
#    ymin = min([h.GetMinimum() for h in histList]);
    ...
    #TODO HARDCODED LIMITS FOR PLOTS
    if "Rcs" in cname:
        if logY:
            ymax = 1
            ymin = 1e-4
    ...
    for i,hist in enumerate(histList):
        ...
        # range
        #Pantelis hist.SetMaximum(10.*ymax)
        hist.SetMaximum(1.*ymax)
        ...
        elif "W + jets" in hist.GetName():
            hist.SetLineColor(kGreen-2)
        elif "t#bar{t} + jets (Pred)" in hist.GetName():
            hist.SetLineColor(kBlue-4)
        elif "T5qqqqWW" in hist.GetName():
            hist.SetFillColorAlpha(kBlack, 0.)
            hist.SetLineWidth(2)
            if "1900" in hist.GetName():
                hist.SetLineColor(kMagenta) #Pantelis before kCyan
            elif "1500" in hist.GetName():
                hist.SetLineColor(kCyan) #Pantelis before kCyan
            elif "1900" in hist.GetName():
                hist.SetLineColor(kGreen) #Pantelis before kCyan
            elif '2150'  in hist.GetName():
                hist.SetLineColor(kRed) #Pantelis before kCyan
            elif '2100'  in hist.GetName():
                hist.SetLineColor(kOrange) #Pantelis before kCyan
            elif '2150'  in hist.GetName():
                hist.SetLineColor(kYellow) #Pantelis before kCyan
            elif '1050'  in hist.GetName():
                hist.SetLineColor(kSpring) #Pantelis before kCyan
            elif '1800'  in hist.GetName():
                hist.SetLineColor(kAzure) #Pantelis before kCyanzbb
            elif '2400'  in hist.GetName():
                hist.SetLineColor(kBlue) #Pantelis before kCyan
            elif '2550'  in hist.GetName():
                hist.SetLineColor(kPink) #Pantelis before kCyan
            else:
                hist.SetLineColor(kCyan) #Pantelis before kMagenta
            hist.Draw(plotOpt+"hist")
            leg.AddEntry(hist,getSampLabel(hist.GetTitle()),"l")
        else:
            if len(histList) < 3:
        ...
        if i == 0:
            ...
            if len(marks) != 0:
                ...
                for i,mark in enumerate(marks):
                    pos = axis.GetBinLowEdge(mark)
                    line = TLine(pos,ymin,pos,1.5*ymax)
                    #line.SetName("line_mark_"+str(mark))
                    line.SetLineStyle(3)
                    line.Draw("same")
                    _lines.append(line)
