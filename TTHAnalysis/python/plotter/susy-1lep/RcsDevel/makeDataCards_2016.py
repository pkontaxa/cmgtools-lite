...
def readSystFile():
    ...
    with open('sysTable_2016.dat',"r") as xfile:  
...
def printABCDCard(yds, ydsObs, ydsKappa, ydsSigSys):
    ...
    for i,bin in enumerate(bins):
        ...
        #If we don't do you signal systematics just assing 20% flat
            ...
            datacard.write('sigSyst_2016 lnN  ' + (' ' * (28))  + before + " 1.2 " + after + "\n")
        ...
        for syst in systDict[bin]:
            datacard.write('A'+syst[1]+'_2016'+' lnN '+ ( ' ' * 32) +(" ".join(([kpatt % systDict[bin][syst] if ('SR_MB','data') == (x.replace('_predict',''),y) else '    -   ' for (x,y) in zip(catNames,sampNames)])))+"\n")
        ...
        for yd,p in zip(ydsKappa[bin], params):
            ...
            if 'QCD' in yd.name: name = p+yd.name +'_'+ Label +'_2016'
        ...
        formula = '(@0*@1/@2*@3)'
        paramIn = 'beta_' + betaName +'_2016' + ',gamma_' + gammaName +'_2016' + ',delta_' + deltaName +'_2016' + ',kappa_' + bin +'_2016'
        if 'beta' in addParam and 'delta' in addParam: 
            formula = formula.replace('@0','(@0-@4)').replace('@2','(@2-@5)')
            paramIn = paramIn +',beta' + betaQCDname +'_2016' + ',delta' + deltaQCDname +'_2016'
        elif addParam == 'beta':
            formula = formula.replace('@0','(@0-@4)')
            paramIn = paramIn +',beta' + betaQCDname+'_2016'
        elif addParam == 'delta': 
            formula = formula.replace('@2','(@2-@4)')
            paramIn = paramIn +',delta' + deltaQCDname +'_2016'

            
        for yd,p in zip(ydsObs[bin], params):
            postFix = bin + '_' + yd.cat
            if p == 'alpha':
                datacard.write(p + '_' + yd.label +'_2016' + ' rateParam ' +yd.cat.replace('_predict','') + ' ' + yd.name.replace('background','data') + ' ' + formula + ' ' + paramIn + '\n')
            else:
                datacard.write(p + '_' + yd.label +'_2016' + ' rateParam ' +yd.cat.replace('_predict','') + ' ' + yd.name.replace('background','data')  + ' ' + str(round(yd.val)) + ' [0,'+str(round(yd.val*3))+'] \n')
...
#        print [name for name in ydsSys.samples if ("syst" in name and "mGo1500_mLSP1000" in name)]
    ...
    pattern = "TEST_2016_Full_25March2021_NEW_DL_CORRECTIONS_LIGHT/*/merged/LT*NJ68*"
    yds6.addFromFiles(pattern,("lep","sele")) 
    pattern = "TEST_2016_Full_25March2021_NEW_DL_CORRECTIONS_LIGHT/*/merged/LT*NJ9i*"
