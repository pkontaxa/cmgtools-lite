...
if __name__ == "__main__":
    ...
    parser.add_option("--range", dest="range", type="float" , nargs=2, default=(0,0), help="Fixing the range to beautify Crissies awesome signal plots")
    addPlotMakerOptions(parser)
    parser.add_option("--prefix", dest="prefix", type="string", default="", help="")
    parser.add_option("--legend", dest="legend", type="string", default="TL", help="")
    ...
    for O,MLD in ("prefit","prefit"), (options.prefix+"postfit_b","fit_b"): #, ("postfit_s","fit_s"):
      ...
      processes = [p for p in reversed(mca.listBackgrounds())]
     ...
     ##--------- RC modified to add signal-----------
      hsignal=[]
      signal = [s for s in mca.listSignals()] 
      for s in signal:
          hsig_tmp = infile.Get(var+"_"+s)
          hsig_tmp.SetFillStyle(0)
          hsig_tmp.SetLineColor(hsig_tmp.GetFillColor())
          hsig_tmp.SetLineWidth(4)
          hsig_tmp.SetLineColor(46)
          #print "The histogram signal name is %s_%s" % (var,s)
          if not hsig_tmp: 
              print "Missing %s_%s" % (var,s)
              continue
          if s in plots:
              plots[s].Add(hsig_tmp)
          else: 
              plots[s] = hsig_tmp
          hsig_tmp.SetName(var+"_"+s)
          hsignal.append(hsig_tmp)            
     ##-----------

      for p in processes:
        pout = mergeMap[p] if p in mergeMap else p
        h = infile.Get(var+"_"+pout)
        #print "The histogram name is %s_%s" % (var,pout) ## RC
        if not h: 
        ...
        if pout in plots:
            ...
            stack.Add(h)
      htot = hdata.Clone(var+"_total_background")
      htotpf = mldir.Get(channel+"/total_background")
      ...
      if options.range != (0,0): htot.GetYaxis().SetRangeUser(options.range[0], options.range[1])
...
#      leg = doLegend(plots,mcap,corner=options.legend,textSize=options.legendFontSize,cutoff=0.0001)
      if options.showMCError:
          leg = doLegend(plots,mcap,corner=options.legendCorner,textSize=0.045,cutoff=0.0001,totalError=totalError)
      else:
          leg = doLegend(plots,mcap,corner=options.legendCorner,textSize=0.045,cutoff=0.0001)
...
#      if "/4l" in args[2]:
#            lspam += ", 4l channel"
      doTinyCmsPrelim(hasExpo = False,textSize=(0.045 if doRatio else 0.033)*options.topSpamSize, xoffs=-0.03,
                      textLeft = options.lspam, textRight = options.rspam, lumi = options.lumi)
      ## Draw relaive prediction in the bottom frame
      p2.cd() 
      rdata,rnorm,rnorm2,rline = doRatioHists(PlotSpec(var,var,"",{}),plots,htot, htot, maxRange=options.maxRatioRange, fitRatio=options.fitRatio, fixRange=options.fixRatioRange,showStatTotLegend=True)
      c1.cd()
